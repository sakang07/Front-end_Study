...
elModal.remove(); // 삭제했어도 선택자는 남아 있으므로 사용 가능
var elCardModal;

// 함수 --------------------------------------------------------
var fnMakeCard = function(n){
  // cloneNode : 기존 node를 복제
...
var fnClickLi = function(e){
  ...
  // 클릭시 수행하는 기능
  // 1. .card_modal 생성
  // 2. .card_modal 영역 나타나기(on)
  // 3. .card_modal 내부의닫기버튼 클릭시 사라지고 삭제

  elConBox.append(elModal); // 삭제했던 요소(.card_modal) 다시 삽입
  elCardModal = elConBox.querySelector('.card_modal');
  var elCloseBtn = elCardModal.querySelector('button');
  elCardModal.classList.add('on'); // .card_modal 영역 나타나기
