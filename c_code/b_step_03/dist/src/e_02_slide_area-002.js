...
*/
// ==========================================
/** 시나리오 - 5.2 (5.2부터 수행하고 5.1처리)
 * + 인디케이터 처리 수행 +
 * 5.2.1 클릭시 해당 순번 파악
 * 5.2.2 파악된 광고슬라이드 해당 위치로 이동(공통변수)
 ...
 * 광고 자체의 링크 tabIndex = -1
 * 마우스 클릭/indicator처리/다음/이전버튼으로 화면이 나타나는 시점에서 tabIndex = 0
 * indicator 기준 클릭이 아닌 focus 처리시 광고화면이 이동, 엔터키를 누르면 해당하는 광고의 위치로 이동 
*/
// ==========================================
// 1. navigation
// 2. modal window
// 3. tab menu
// 4. 광고 슬라이드 setInterval (일정시간마다 넘어가는 기능)
...
(()=>{
  ...
  const slideLen = elSlideLi.length; // 슬라이드 복제 전 갯수 파악 변수


  // 기능처리 --------------------------------
  // -------------
  // 5.2 (cloneNode는 indicator 목록에 포함되지 않는다)
  ...
  // 일정시간마다 광고 슬라이드 이동 수행(다음 버튼 클릭과 동일기능)
  let slideGo;
  ...
  // 광고영역에 마우스 올리면, 광고 슬라이드 일시정지
  // mouseenter: 한번 올라가면 끝 / mouseover: 끊임없이 인식
  elViewBox.addEventListener('mouseenter', e => {
    fnSlidePause();
  });

  // 광고영역에서 마우스 벗어나면 광고 슬라이드 다시 재생
  // mouseleave / mouseout
  elViewBox.addEventListener('mouseleave', e => {
    fnSlideMove();
  });
